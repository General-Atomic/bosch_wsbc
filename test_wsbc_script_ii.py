...
# CONFIGURATION
# -----------------------------------------------------------------------------
# Thresholds for progress bar colors (scaled for new max values)
OHB_THRESHOLDS = [
    (None, 75,  "#e74c3c"),   # Red   (0–75)
    (76,  188, "#f1c40f"),    # Yellow (76–188)
    (189, None, "#2ecc71")    # Green (189+)
]
WS_THRESHOLDS = [
    (None, 82,  "#e74c3c"),   # Red   (0–82)
    (83,  328, "#f1c40f"),    # Yellow (83–328)
    (329, None, "#2ecc71")    # Green (329+)
]

OHB_PROGRESS_MAX = 375  # Maximum value displayed on OHB progress bar
WS_PROGRESS_MAX = 820   # Maximum value displayed on WS progress bar
...
# Graph time window & update rate
GRAPH_WINDOW_SECONDS = 2 * 60 * 60   # last 2 hours
GRAPH_UPDATE_MS = 5000               # update graph every 5 seconds
...
# -----------------------------------------------------------------------------
# Trend Graph (OHB / WS over time, x = time, y = 0–100% stacked)
# -----------------------------------------------------------------------------
class TrendGraph:
    """
    100% stacked area chart using Tkinter Canvas.

    - OHB and WS plotted as percentages of total (OHB + WS).
    - X-axis: last 2 hours of time, labeled HH:MM.
    - Y-axis: 0%, 50%, 100%.
    - Regions are shaded, with thicker boundary lines.
    """
    def __init__(self, parent, width=780, height=260, title="Relative Buffer Share (OHB / WS)"):
        self.width = width
        self.height = height

        # Rolling window – we'll prune by time, but this is a safeguard too
        self.max_points = 10 * 60 * 60 // 5  # not critical; just "large enough"
    ...
    def _prune_old(self):
        """
        Keep only points within the last GRAPH_WINDOW_SECONDS.
        """
        if not self.history_times:
            return
        cutoff = self.history_times[-1] - GRAPH_WINDOW_SECONDS
        while self.history_times and self.history_times[0] < cutoff:
    ...
    def update_series(self, ohb, ws, timestamp=None):
        ...
        # Safety max length
        if len(self.history_times) > self.max_points:
            self.history_times.pop(0)
            self.history_ohb.pop(0)
            self.history_ws.pop(0)
    ...
    def _redraw(self):
        """
        Redraws the 100% stacked area chart.
        """
        # Clear previous plotted lines and fills
        self.canvas.delete("graph_line")
        self.canvas.delete("graph_axis")
        self.canvas.delete("graph_fill")

        if len(self.history_times) < 2:
            return

        # Y is fixed 0–100% for 100% stacked chart
        max_val = 1.0
        ...
        # Y ticks: 0%, 50%, 100%
        for pct, label in ((0.0, "0%"), (0.5, "50%"), (1.0, "100%")):
            y = self.plot_bottom - pct * y_span
        ...
        def x_pos(t):
            return self.plot_left + ((t - t_min) / (t_max - t_min)) * x_span

        # Build % series
        ohb_pct = []
        ws_pct = []
        for o, w in zip(self.history_ohb, self.history_ws):
            total = o + w
            if total <= 0:
                ohb_pct.append(0.0)
                ws_pct.append(0.0)
                ...
                op = o / float(total)
                ohb_pct.append(op)
                ws_pct.append(1.0 - op)

        # Build OHB polygon (bottom → ohb_pct)
        ohb_poly = []
        for t, op in zip(self.history_times, ohb_pct):
            x = x_pos(t)
            y = self.plot_bottom - op * y_span
            ohb_poly.append((x, y))
        # Close polygon down to baseline
        ohb_poly_baseline = [(x_pos(self.history_times[-1]), self.plot_bottom),
                             (x_pos(self.history_times[0]), self.plot_bottom)]
        ohb_poly_full = ohb_poly + ohb_poly_baseline

        flat_ohb = [coord for xy in ohb_poly_full for coord in xy]
        if len(flat_ohb) >= 6:
            self.canvas.create_polygon(*flat_ohb,
                                       fill="#f8cfa2",  # light orange shade
                                       outline="",
                                       tags="graph_fill")

        # Build WS polygon (ohb_pct → 100%)
        ws_poly = []
        for t, op in zip(self.history_times, ohb_pct):
            # lower boundary = ohb_pct
            x = x_pos(t)
            y_low = self.plot_bottom - op * y_span
            ws_poly.append((x, y_low))
        # Top boundary is 100% line
        ws_poly_top = [(x_pos(self.history_times[-1]), self.plot_top),
                       (x_pos(self.history_times[0]), self.plot_top)]
        ws_poly_full = ws_poly + ws_poly_top

        flat_ws = [coord for xy in ws_poly_full for coord in xy]
        if len(flat_ws) >= 6:
            self.canvas.create_polygon(*flat_ws,
                                       fill="#b9e6c5",  # light green shade
                                       outline="",
                                       tags="graph_fill")
        ...
        for t, op in zip(self.history_times, ohb_pct):
            ...
            ohb_line.extend((x, y))
        if len(ohb_line) >= 4:
            self.canvas.create_line(*ohb_line,
                                    fill="#e67e22",
                                    width=3,
                                    smooth=False,
                                    tags="graph_line")

        # WS upper boundary (100% line)
        self.canvas.create_line(self.plot_left, self.plot_top,
        ...
        # X-axis ticks every ~30 minutes over last 2 hours
        # (5-second sampling is independent of tick marks)
        # Adjust step if you want finer / coarser labels.
        tick_step = 30 * 60  # 30 minutes
        first_tick = math.floor(t_min / tick_step) * tick_step
        tick_t = first_tick
...
graph_panel = TrendGraph(bottom_frame, width=780, height=260,
                         title="Relative Buffer Share (OHB / WS)")
...
def read_inputs_and_update():
    ...
    # Graph is updated on its own timer (graph_time_tick)

    # Schedule next poll
    main.after(POLL_INTERVAL_MS, read_inputs_and_update)

# -----------------------------------------------------------------------------
# Time-domain graph updater (one point every GRAPH_UPDATE_MS)
# -----------------------------------------------------------------------------
def graph_time_tick():
    """
    Append current OHB/WS values to the graph on a fixed interval,
    so the X-axis is true time (time domain) with reduced CPU load.
    """
    now_ts = time.time()
    graph_panel.update_series(sum_ohb, sum_ws, timestamp=now_ts)
    main.after(GRAPH_UPDATE_MS, graph_time_tick)
